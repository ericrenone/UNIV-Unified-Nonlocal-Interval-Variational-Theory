...
**[T — Theorem 1.4, KQOM]** *(Dimensional Permeability Product — DPP).* If (S₁, ≼₁) and (S₂, ≼₂) are SP then (S₁ × S₂, ≼_componentwise) is SP with ordinal ceiling o*(ℕ²) = ω². *Proof:* given any infinite sequence in S₁ × S₂, SP of S₁ yields i < j with aᵢ ≼₁ aⱼ; within the infinite sub-sequence satisfying this, SP of S₂ yields k < ℓ with bₖ ≼₂ bₗ; (k,ℓ) satisfies both coordinates. Induction gives the general statement. ∎
...
**[T — Theorem 2.3, KQOM]** *(Ordinal Termination.)* Under (OC1), (OC2), and the additional condition that δ(s_{t+1}) < δ(s_t) strictly at every step, the trajectory reaches the Terminal Boundary in at most δ(s₀) steps.

> **Note on the strict-descent condition.** The strict descent δ(s_{t+1}) < δ(s_t) is a separate dynamical assumption beyond (OC1)–(OC2); it is not a consequence of WQO alone. For deterministic systems it must be verified from the update rule U. For stochastic updates, the appropriate replacement is the supermartingale condition 𝔼[δ(s_{t+1}) | s_t] ≤ δ(s_t) − c for some c > 0, which implies almost-sure termination by the optional stopping theorem. These two variants — deterministic strict descent and stochastic Lyapunov — are logically independent; the theorem applies to whichever is established for the system at hand.
...
#### III.3 The Four-Language Characterization of λ₁ > 0

**[T — Theorem 3.4, KQOM]** *(Spectral–Stochastic–Algebraic–Arithmetic Characterization, within the ℒ_JL domain.)* Under (A1)–(A5), the four conditions below characterize λ₁(ℒ_JL) > 0 with the proof status and domain restrictions stated individually for each.

**(I)** λ₁(ℒ_JL) > 0. **[T]** — Primary criterion; immediate from the variational formula and the self-adjointness established in Theorems 3.1–3.3.

**(II)** The Poincaré inequality holds: there exists C_P > 0 such that for all φ ∈ H¹(ℬ, D_s),

```
∫_ℬ |φ − ⟨φ⟩_μ|² dμ  ≤  C_P · ∫_ℬ ⟨D_s ∇φ, ∇φ⟩ dvol_ℬ
```

**[T]** (I) ↔ (II) exactly, with λ₁ = 1/C_P^{opt}. This is the definition of spectral gap for the self-adjoint form 𝔞. The computable sufficient condition Γ = ‖∇_ℬ 𝒮̄‖²_{L²(μ)} / Tr(D_s) > 1 implies (I) via the Bakry–Émery Γ₂ curvature criterion (Γ₂ ≥ κ·Γ implies λ₁ ≥ κ), but is not in general equivalent to (I).

> **Stochastic connection.** For continuous-time diffusions with generator ℒ_JL, (I)↔(II) implies exponential L²(μ)-mixing: ‖e^{-tℒ_JL} − Π‖_{L²→L²} ≤ e^{-λ₁ t}, where Π is the projection onto the ground state. For discrete-time mini-batch SGD, the generator approximates ℒ_JL up to O(η) corrections in the diffusion limit (step size η → 0 at fixed noise variance). The ordinal supermartingale condition 𝔼[δ(s_{t+1}) | s_t] ≤ δ(s_t) is a Lyapunov stability condition for the discrete chain that is implied by (I) in the diffusion limit via Foster–Lyapunov theory, but constitutes a separate, weaker statement at finite step size.

**(III)** C_α = ‖μ_g‖² / Tr(Σ_g) > 1, where μ_g = 𝔼[∇L] denotes the expected gradient and Σ_g = Cov[∇L] denotes gradient covariance. **[T in the large-batch limit; asymptotic indicator at finite batch]** In the large-batch limit, μ_g → ∇_ℬ𝒮̄ and Σ_g → D_s pointwise, so C_α → Γ. Under (A3), the Bakry–Émery argument then gives: C_α > 1 (large-batch limit) → (I). The converse — (I) → C_α > 1 — holds in the large-batch limit under the additional spectral dominance assumption that λ_max(D_s) / ‖∇_ℬ𝒮̄‖ is uniformly bounded; see GAME-O5. The identification (I) ↔ (III) is therefore exact in the large-batch limit under spectral dominance, and an asymptotic indicator at finite batch.

**(IV)** The Möbius inversion Mₙ = Σ_{k≤n} μ(k,n)·F_k converges in L²(ℬ, μ), where μ(k,n) is the Rota–Hall Möbius function on the chain poset of loss values. **[T: (I) → (IV); [C]: (IV) → (I)]** Direction (I) → (IV): under λ₁ > 0, the spectral expansion ρ(·,t) = Σ_n cₙ e^{-λₙt} φₙ in L²(μ) converges exponentially; the Möbius inversion on the loss-value poset recovers ρ_∞ = φ₁ from accumulated flows via the ground-state transform because the Möbius function on a chain poset equals the finite-difference operator, and the L² convergence of F_k to ∇_ℬ𝒮̄ · φ₁ then gives L² convergence of Mₙ. Direction (IV) → (I) requires in addition that divergent Mₙ witnesses a null eigenvalue; this is conjectural and listed as GAME-O7.

> **Summary.** The equivalences (I)↔(II) and (I)↔(III) [large-batch, spectral dominance] are proven within the ℒ_JL domain under (A1)–(A5). Condition (IV) is a proven consequence of (I) and a conjectural characterization in the reverse direction.

| λ₁ Sign | C_α (large-batch) | Dynamical Behavior |
|---------|--------------------|-------------------|
...
**Implication (conditional on UNIV-H1).** If UNIV-H1 holds for a given quantum system, the Spectral–Stochastic–Algebraic–Arithmetic Characterization applies to K_A, and sign(λ₁(K_A)) is the phase oracle. The sign inversion between domains — λ₁(ℒ_JL) > 0 signals learning generalization; λ₁(K_A) < 0 signals nontrivial topological order — is not a contradiction: both operators are identified under UNIV-H1, and the physical interpretation of which sign is favorable differs between domains.
...
The symmetric term (proportional to c) is invisible to supplementary-angle-averaged diagnostics. The chiral term (c₋/4)·cot(θ) is odd under θ → π − θ, invisible to all area-law and topological diagnostics, and computable from short-range corner structure alone.
...
*Step 3:* Take the trace in the vacuum. Non-chiral contributions from c_L and c_R cancel in the commutator at leading order (the commutator selects c_L − c_R via the relative sign of the holomorphic and antiholomorphic contributions). Result:
...
**Definition.** For spatial sub-region V with boundary ∂V, the Hodge-dual surface Σ* carries F̃ = ½ε_μνρσ F^ρσ, defined by ∫_{Σ} F̃ = Q_color(V). The UNIV mapping identifies ∂A ↔ Σ.

**[V — Verified in U(1)_k Chern–Simons, disc A]** K_A is the chiral boson Hamiltonian on ∂A = S¹; the Hodge dual F̃ = F_{01} = E is a scalar; Σ = ∂A exactly.
...
where r_ν are the Farey fractions of order n in sorted order.
...
**Categorical framework.** The results in this section assume the bulk topological order is fully described by a unitary modular tensor category 𝒞. All statements about anyons, braiding, and boundary conditions are statements within this categorical framework.

**[T — Kitaev 2006; Kong–Wen 2014]** Within the modular tensor category framework, a gapped boundary exists if and only if the anyon model admits a *Lagrangian subgroup*: a set of bosonic anyons {bᵢ} with trivial mutual braiding generating the full topological order. The forward direction (Lagrangian subgroup → gapped boundary) is constructive: condense the anyons in the Lagrangian subgroup. The reverse direction (gapped boundary → Lagrangian subgroup) follows from Kong–Wen 2014 applied to the module category of boundary conditions over 𝒞.

**[T]** Each anyon bᵢ must satisfy e^{2πi h(bᵢ)} = 1. With chiral central charge c₋, topological spins satisfy:

```
h(bᵢ) ≡ hᵢ^{(0)} + (c₋/8) · Qᵢ²   (mod 1)
```

For a Lagrangian subgroup with Qᵢ ∈ {0,1}, the bosonic condition requires:

```
c₋ / 8  ∈  ℤ   ⟺   c₋  ≡  0   (mod 8)
```

**[T — Gapping Obstruction Theorem]** Within the modular tensor category framework, a topological phase with c₋ ≢ 0 mod 8 admits no gapped boundary and no magnetic condensate.

#### VII.2 The (𝔠_tot)_min Obstruction and Geometric Twist

**[T]** (𝔠_tot)_min = min{c_L + c_R : consistent unitary boundary CFT} > 0 whenever c₋ ≢ 0 mod 8. *Proof:* unitarity gives c_L, c_R ≥ 0, so c_L + c_R ≥ |c_L − c_R| = |c₋|; for purely chiral theories (𝔠_tot)_min = |c₋|; in general (𝔠_tot)_min ≥ |c₋|/2 > 0 without further assumptions. ∎
...
**[T — within the modular tensor category framework]:**
...
**[T — Chiral UNT Protection Theorem]** For a topological phase Π with c₋ ≢ 0 mod 8, within the modular tensor category framework:
...
*Proof of (1), (2), (4):* A magnetic condensate requires a Lagrangian subgroup, which requires c₋ ≡ 0 mod 8, violated by hypothesis. Local perturbations preserving the bulk gap cannot create a Lagrangian subgroup from nothing; the anyon model is a bulk invariant determined by 𝒞. *Proof of (3):* conditional on UNIV-C1. *Proof of (5):* c_L + c_R ≥ |c_L − c_R| = |c₋| by unitarity of the boundary CFT, so (𝔠_tot)_min ≥ |c₋|/2 > 0 without further categorical assumptions. ∎
...
#### Domain-Internal Chains

**Within the ℒ_JL domain (all [T] under A1–A5, with domain scoping as established in Phase III):**

| Label | Condition | Status |
|-------|-----------|--------|
| (I) | λ₁(ℒ_JL) > 0 | [T] |
| (II) | Poincaré inequality with C_P < 1 | [T] ↔ (I) |
| (III) | C_α > 1 | [T] ↔ (I) in large-batch limit + spectral dominance; asymptotic at finite batch |
| (IV) | Möbius inversion Mₙ converges in L² | [T] (I)→(IV); [C] (IV)→(I) |

**Within the CFT/TQFT + modular tensor category framework:**

| Label | Condition | Status |
|-------|-----------|--------|
| (V) | (𝔠_tot)_min = 0 | [T] ↔ c₋ ≡ 0 mod 8 |
| (VI) | A_min(Σ*) = 0 | [V] U(1) CS; [C] UNIV-C3 in general |
| (VII) | J(A,B) = 0 | [V] free boson; [C] UNIV-C1 in general |
| (VIII) | c₋ ≡ 0 (mod 8) | [T] within modular tensor category framework |
...
| (IV) ↔ (VI) | UNIV-C3 + GAME-O7 | [C]+[C] |
| (II) ↔ (V) | UNIV-H1 + diffusion limit identification | [H] |
...
**[T]** Every topological phase with c₋ ≢ 0 mod 8 has a permanently gapless boundary admitting no magnetic condensate (within the modular tensor category framework).
...
| UNIV-C4 | Farey-distributed trajectories have discrepancy O(n^{-1/2+ε}) conditional on RH | Franel–Landau criterion for flow sequences | Analytic number theory + stochastic analysis |
...
| GAME-O7 | Converse of (I)→(IV): Möbius convergence implies λ₁ > 0 | Connecting divergent Möbius sums to null eigenvalues | Spectral theory of diffusion operators + Rota–Hall Möbius inversion |
...
| 10 | (I)↔(II): spectral gap ↔ Poincaré inequality | [T] | Rayleigh quotient; self-adjoint form theory |
| 11 | (I)↔(III) in large-batch limit + spectral dominance | [T] + [C for converse at finite batch] | Bakry–Émery + gradient statistics |
| 12 | (I)→(IV): λ₁ > 0 implies Möbius convergence | [T] | Ground-state spectral expansion + Rota–Hall |
| 13 | Li-Haldane correspondence | [T] | Li–Haldane 2008 |
| 14 | Corner universality: f(θ) encodes c₋ | [T] | Bueno–Myers 2015; Faulkner et al. 2016 |
| 15 | J(A,B) = πc₋/3 in free chiral boson | [V] | Explicit modular Hamiltonian calculation |
| 16 | Gapped boundary ↔ Lagrangian subgroup (MTC framework) | [T] | Kitaev 2006; Kong–Wen 2014 |
| 17 | c₋ ≡ 0 mod 8 necessary for gapping | [T] | Topological spin argument; anomaly inflow |
| 18 | CCC: λ_max(H) ≲ C₀/(q*)² | [V] | Under Assumptions S and E (GAME) |
| 19 | Farey-PAC-Bayes bound L_test − L_train ≲ q*√(d/n) | [V] | Given CCC + McAllester 1999 |
| 20 | UNIV-H1: K_A ↔ ℒ_JL (general) | [H] | Verified: free boson, U(1) CS |
| 21 | UNIV-C2: continuum flux satisfies (OC1)–(OC2) | [C] | No Yang-Mills result assumed |
| 22 | Franel–Landau ↔ Riemann Hypothesis | [T] | Franel 1924; Landau 1924 |
...
ZF Axioms
    ...
    │         │   [distinct from supermartingale condition; both forms stated]
    ...
    │       ├─→ (I)↔(II): spectral gap ↔ Poincaré inequality [T]
    │       ├─→ (I)↔(III): large-batch + spectral dominance [T+C]
    │       └─→ (I)→(IV): Möbius convergence as consequence [T]
    │           (IV)→(I): converse open [GAME-O7]
    │
    ...
    │
    └─→ Chiral Obstruction / Gapping Theory [T within MTC framework]
            ├─→ Gapping ↔ Lagrangian subgroup [T]
            ├─→ c₋ ≡ 0 mod 8 necessary [T]
            ├─→ (𝔠_tot)_min ≥ |c₋|/2 > 0 when c₋ ≢ 0 mod 8 [T]
...
C_α                 ‖μ_g‖² / Tr(Σ_g)                           [T] within ℒ_JL
                    exact indicator in large-batch limit         domain; asymptotic
                    + spectral dominance                         at finite batch
...
(𝔠_tot)_min          0 iff c₋ ≡ 0 mod 8                         [T] within MTC framework
────────────────────────────────────────────────────────────────────────────────
```
...
**Proven foundations:** ZF · Dickson (1913) · Higman (1952) · Kruskal (1960) · Nash-Williams (1963) · Robertson–Seymour (1985–2004) · Kato (1966) · Bakry–Émery (1985) · Bisognano–Wichmann (1975) · Li–Haldane (2008) · Kitaev (2006) · Cauchy (1816) · Hurwitz (1891) · Franel–Landau (1924)

**Active conjectures:** UNIV-H1 · UNIV-C1 · UNIV-C2 · UNIV-C3 · UNIV-C4 · GAME-O4 · GAME-O5 · GAME-O7
